...
/*!*************************************************************************************************
 callback handler for the packet received
***************************************************************************************************/
...
)
{
  ...
  uint16_t yData;
  uint16_t zData;
  uint32_t pMyPayloadSize = 18;
  uint8_t pMySessionPayload[18] = {0};
  pMySessionPayload[0] = 'X';
  pMySessionPayload[1] = '=';
  pMySessionPayload[6] = 'Y';
  pMySessionPayload[7] = '=';
  pMySessionPayload[12] = 'Z';
  pMySessionPayload[13] = '=';
  ...
  else//if its dumb enough and cant read the sensor, send zeros
  {
	  xData = 0;
	  yData = 0;
	  zData = 0;
  }
  itoa(xData, &pMySessionPayload[2], 10);
  itoa(yData, &pMySessionPayload[8], 10);
  itoa(zData, &pMySessionPayload[14], 10);
  uint8_t * pointer = NULL;
  pointer = pMySessionPayload;
  ...
  if (gCoapConfirmable_c == pSession->msgType)
  {
    ...
    {
    	COAP_Send(pSession, gCoapMsgTypeAckSuccessContent_c, pointer, pMyPayloadSize);
    }
  }
